...
// The zBTN block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zBTN is a hash of the serial
...
{
    //Create an output returning the zBTN that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZBTNOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zBTN output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zBTN", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZBTNOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zBTN output", __func__);
...
{
    CzBTNTracker* zbtnTracker = pwallet->zbtnTracker.get();
    CMintMeta meta;
    if (!zbtnTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zbtnTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a BTN stake is the outpoint
