...
CzBTNWallet::CzBTNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbtn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBTNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBTNSeed_deprecated()) {
                    LogPrintf("%s: Updated zBTN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbtn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbtn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBTNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBTNCount(nCountLastUsed);
    else if (!walletdb.ReadZBTNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBTNWallet::Lock()
{
    seedMaster = 0;
}

void CzBTNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBTNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBTN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBTNWallet::LoadMintPoolFromDB()
{
...
void CzBTNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBTNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBTNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbtnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBTNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBTN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbtnTracker which also adds to database
    pwalletMain->zbtnTracker->Add(dMint, true);
...
void CzBTNWallet::SeedToZBTN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBTNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBTNWallet::GenerateDeterministicZBTN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBTNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBTNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBTNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
