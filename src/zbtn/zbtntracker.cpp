...
CzBTNTracker::CzBTNTracker(std::string strWalletFile)
{
...
CzBTNTracker::~CzBTNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBTNTracker::Init()
{
...
bool CzBTNTracker::Archive(CMintMeta& meta)
{
...
bool CzBTNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBTNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBTNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBTNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzBTNTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzBTNTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzBTNTracker::GetSerialHashes()
{
...
CAmount CzBTNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBTNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBTNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBTNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBTNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBTNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBTNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBTNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBTNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBTNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBTNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBTNWallet* zBTNWallet)
{
    bool iszBTNWalletInitialized = (NULL != zBTNWallet);
    CMintMeta meta;
    ...
    if (! iszBTNWalletInitialized)
        zBTNWallet = new CzBTNWallet(strWalletFile);
    meta.isSeedCorrect = zBTNWallet->CheckSeed(dMint);
    if (! iszBTNWalletInitialized)
        delete zBTNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBTNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBTNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBTNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBTNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBTNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBTNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBTNWallet* zBTNWallet = new CzBTNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBTNWallet);
        }
        delete zBTNWallet;
        LogPrint("zero", "%s: added %d dzbtn from DB\n", __func__, listDeterministicDB.size());
    }
