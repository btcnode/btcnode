...
public:
    ...
    bool WriteZBTNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBTNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBTNSeed_deprecated(uint256& seed);
    bool EraseZBTNSeed();
    bool EraseZBTNSeed_deprecated();

    bool WriteZBTNCount(const uint32_t& nCount);
    bool ReadZBTNCount(uint32_t& nCount);
