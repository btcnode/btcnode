...
#include "ui_zbtncontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zbtnTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zbtnTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zBTN.";
            else if (!mint.isSeedCorrect)
                strReason = "The zBTN seed used to mint this zBTN is not the same as currently hold in the wallet";
            else
