...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZBTNModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zbtnTracker->HasMintTx(hash))) {
            //zBTN stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZBTN;
...
bool IsZBTNType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZBTN:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZBTN || type == TransactionRecord::MNReward) {
