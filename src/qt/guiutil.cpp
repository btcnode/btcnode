...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Btcnode address (e.g. %1)").arg("C7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no Btcnode: URI
...
{
    // Convert btcnode:// to btcnode:
    //
    //    Cannot handle this later, because btcnode:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open btcnode.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Btcnode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Btcnode.lnk
...
{
    return GetAutostartDir() / "btcnode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a btcnode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Btcnode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the btcnode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add btcnode app to startup item list
